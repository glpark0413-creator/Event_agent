...
이벤트 제목·날짜·명칭 수정이 모두 끝난 직후 실행한다.

#### 1단계 — 보상 명칭 입력 (AskUserQuestion)

**[질문]** — `AskUserQuestion` 호출:
- 질문: "이벤트에 사용할 신규 보상 명칭이 있나요? 있으면 직접 입력해주세요."
- 옵션 (2개):
  1. `보상 명칭 직접 입력` — 새롭게 사용할 보상 이름을 텍스트로 입력
  2. `없음` — 보상 명칭 변경 없이 수량 자동 조정만 진행

**`보상 명칭 직접 입력` 선택 시**:
```
추가·변경할 보상 명칭을 쉼표로 구분해 입력해주세요.
예) 다이아몬드, 골드 박스, 슈퍼 픽업팩
```
입력된 명칭은 생성된 xlsx의 보상 셀에서 기존 보상 명칭을 찾아 치환한다.

> 치환 기준: 입력 명칭과 가장 유사한 기존 보상 항목을 매핑한다.
> 매핑이 불분명하면 에스컬레이션하고 임의로 결정하지 않는다.

---

#### 2단계 — 보상 수량 자동 조정

1단계 응답(보상 명칭 입력 또는 없음)과 무관하게 항상 실행한다.

**실행 순서**

1. **기존 탭 전체 스캔** — xlsx 소스 파일에서 날짜형(YYMMDD) 탭을 모두 읽는다.
   보상 관련 셀(수치 + 단위 패턴, 예: `"다이아 50개"`, `"50,000 골드"`)을 수집한다.

2. **보상 유형별 평균 산출** — 수집한 수치를 보상 유형(다이아, 골드, 박스 등)별로 분류하고
   탭별 평균값을 계산한다.
   ```
   [보상 수량 분석 결과]
   ─────────────────────────────────────────
...
3. **요청자 확인** — `AskUserQuestion` 호출:
   - 질문: "분석된 평균 수량으로 보상을 자동 수정할까요?"
   - 옵션 (3개):
     1. `평균 수량 적용` — 분석된 평균값으로 생성 탭의 보상 수량 일괄 수정
     2. `직접 수정` — 보상 유형별로 수량을 직접 입력
     3. `수량 유지` — 수량 변경 없이 현재 값 유지

4. **응답별 처리**:

   | 응답 | 처리 |
   |---|---|
   | `평균 수량 적용` | 분석된 평균값으로 생성 탭의 보상 수량 셀 일괄 갱신 |
   | `직접 수정` | "보상 유형별 수량을 입력해주세요 (예: 다이아=80개, 골드=30000)" 입력 대기 후 적용 |
   | `수량 유지` | 수량 변경 없이 다음 단계로 |

5. **xlsx 재저장** — 보상 수정 내역을 반영해 output 파일을 덮어쓴다.
